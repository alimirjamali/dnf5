...
TransactionTable::Impl::Impl(ITransaction & transaction) {
    ...
    auto column = scols_table_new_column(*tb, _("Package"), 0.1, SCOLS_FL_TRUNC);
    ...
    // whint == 26 chars on 80 width displays. But is squeezed later if possible
    column = scols_table_new_column(*tb, _("Version"), 0.325, SCOLS_FL_TRUNC);
    header = scols_column_get_header(column);
    ...
    // whint == 26 chars on 80 width displays. But is squeezed later if possible
    column = scols_table_new_column(*tb, _("Repository"), 0.325, SCOLS_FL_TRUNC);
    header = scols_column_get_header(column);
...
void TransactionTable::Impl::print_table() {
    ...
    size_t max_ver_len = 0, max_rep_len = 0;
    for (size_t n = 0; n < scols_table_get_nlines(*tb); n++) {
        ln = scols_table_get_line(*tb, n);
        max_ver_len = std::max(max_ver_len, strlen(scols_cell_get_data(scols_line_get_cell(ln, 2))));
        max_rep_len = std::max(max_rep_len, strlen(scols_cell_get_data(scols_line_get_cell(ln, 3))));
    }
    if (max_ver_len < scols_column_get_whint(scols_table_get_column(*tb, 2)) * 80) {
        scols_column_set_whint(scols_table_get_column(*tb, 2), max_ver_len);
    }
    if (max_rep_len < scols_column_get_whint(scols_table_get_column(*tb, 3)) * 80) {
        scols_column_set_whint(scols_table_get_column(*tb, 3), max_rep_len);
    }
