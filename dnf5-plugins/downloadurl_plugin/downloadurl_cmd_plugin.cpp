...
constexpr const char * PLUGIN_NAME{"downloadurl"};
constexpr PluginVersion PLUGIN_VERSION{.major = 0, .minor = 0, .micro = 1};

constexpr const char * attrs[]{"author.name", "author.email", "description", nullptr};
constexpr const char * attrs_value[]{"Ali Mirjamali", "ali@mirjamali.com", "downloadurl command."};

class DownloadURLCmdPlugin : public IPlugin {
public:
...
std::vector<std::unique_ptr<Command>> DownloadURLCmdPlugin::create_commands() {
    std::vector<std::unique_ptr<Command>> commands;
    commands.push_back(std::make_unique<DownloadURLCommand>(get_context()));
    return commands;
}
...
IPlugin * dnf5_plugin_new_instance([[maybe_unused]] ApplicationVersion application_version, Context & context) try {
    return new DownloadURLCmdPlugin(context);
} catch (...) {
    return nullptr;
}
